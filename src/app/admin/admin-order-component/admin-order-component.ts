...
fetchOrders() {
  this.http.get<any[]>('api/orders').subscribe(data => {
    ...
    this.processingCount = data.filter(o => o.status === 'processing').length;
  ...
  downloadCSV() {
  const headers = ['No', 'Customer Name', 'Phone', 'Product Name', 'Price', 'Qty', 'Payment Method', 'Payment Status', 'Status'];
